...
<input name="photo" type="file"  placeholder="Фото" onChange={(event: React.ChangeEvent<HTMLInputElement>) => { if (event.target.files && event.target.files.length > 0) setImg(event.target.files[0]) }} />
...
<div> <input name="photo" type="file"  placeholder="Фото" onChange={(event: React.ChangeEvent<HTMLInputElement>) => { if (event.target.files && event.target.files.length > 0) setImg(event.target.files[0]) }} /> <input value={nameJewelery} placeholder='author' onChange={(e) => setNameJewelery(e.target.value)} /><button type="button" onClick={openModal} >add</button> <ModalWindowAuth isOpen={isModalOpen} onClose={closeModal} /> </div> ) )
}
